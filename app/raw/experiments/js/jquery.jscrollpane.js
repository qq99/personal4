	...
	$.fn.jScrollPane = function(settings)
	{
		...
		function JScrollPane(elem, s)
		{
			...
			var settings, jsp = this, pane, paneWidth, paneHeight, container, contentWidth, contentHeight,
				...
				originalData; // keeps original data intact s.t. we might restore it on unload()
							// keeping the data in here instead of just pulling it from pane / elem
							// might be a waste of space, but I tolerate it for some reason that is
			...
			function initialise(s)
			{
				...
				if (! elem.hasClass('jScrollPane')) { // not a pane === undefined check, as it can be unloaded/reloaded
					elem.addClass('jScrollPane'); // signifies the plugins DOM-space
					elem.css(
						{
							'overflow': 'hidden',
							'padding': 0
					...
					if (pane === undefined){ // only on object first-load, caching
						originalData = elem.children();
					}
					// oddly we can't cache pane's structure as removal from DOM in unload() causes the object to cease to exist
					pane = $('<div class="jspPane" />').css('padding', originalPadding).append(originalData);
					...
					// ^ this seems like one heck of a mess to me
					// why append when it might make more sense to use elem.html('the plugin')?
				...
				contentHeight = Math.max(pane.outerHeight(), clonedElem.outerHeight());// same trick needed to be applied in my case, unsure why
				tempWrapper.remove();
			...
			function unload(){
				elem.removeClass('jspScrollable');
				elem.removeClass('jScrollPane');// it's no longer a plug-in object
				elem.html(originalData); // put the original data back into its container
				elem.attr("style",""); // remove all styles applied by plug-in
				removeMousewheel();
				removeFocusHandler();
				removeKeyboardNav();
				removeClickOnTrack();
				unhijackInternalLinks();
				settings.autoReinitialise = false; // I think the auto-reinit needs to be better wrapped, hoping this is all I need to disable it
			} // reverts the element back to its original state
			...
			$.extend(
				jsp,
				{
					...
					// Return the element back to its original state, unbinds all mouse/click/etc functions.
					// To return the object back to a jScrollPane, just call reinitialise
					unload: function()
					{
						unload();
					},
