...
(function(){	
	...
	var camera_keydown = function(event){
		...
		if (event.keyCode == 81) // Q
		{
			which += 1;
			if (which > 3) which = 0;
			CAMERA.panTo(corners[which], 2000, 20);
		}
	...
	var bind_camera = function(event){
		bind_screenedges();
		$("#gamearea").bind("mousewheel", camera_mousewheel);
		$(document).bind("keydown", camera_keydown);
	};
	var unbind_camera = function(event){
		unbind_screenedges();
		$("#gamearea").unbind("mousewheel", camera_mousewheel);
	...
	this.init = function(canvas, eye, up, lookat, gameres){
